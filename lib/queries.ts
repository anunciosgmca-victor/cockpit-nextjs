...
export async function fetchLevers(): Promise<Lever[]> {
  const { data, error } = await supabase().from('levers').select('*').eq('workspace', currentWorkspace()).order('created_at');
...
export async function addLever(name: string) {
  const { error } = await supabase().from('levers').insert({ name, workspace: currentWorkspace() });
  if (error) throw error;
}

// ---------- topics ----------
export async function addTopic(input: { lever_id: string; name: string; strategy: string; objective: string }) {
  const { error } = await supabase().from('topics').insert({ ...input, workspace: currentWorkspace() });
  if (error) throw error;
}
...
export async function startMeeting(): Promise<string> {
  const { data, error } = await supabase().from('meetings').insert({ workspace: currentWorkspace() }).select('id').single();
  if (error) throw error;
  return data.id;
}
