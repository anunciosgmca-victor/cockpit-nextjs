...
const BRANDS: Record<'default' | 'grupoordos', BrandConfig> = {
  default: {
    themeClass: '',
    brandLetter: 'M',
    brandTitle: 'Cockpit de Gestão',
    brandSubtitle: 'Marketing & Growth',
  },
  grupoordos: {
    themeClass: 'theme-red',
    brandLetter: 'G',
    brandTitle: 'Cockpit Grupo Ordos',
    brandSubtitle: 'Marketing & Growth',
  },
};
...
export default function CockpitApp({ workspace }: { workspace: 'default' | 'grupoordos' }) {
  const brand = BRANDS[workspace];
  ...
  const appClass = 'app' + (brand.themeClass ? ' ' + brand.themeClass : '');

  if (loading) {
    return <div className={appClass}><div className="empty-state"><h3>Carregando cockpit...</h3></div></div>;
  }
  ...
  return (
    <div className={appClass}>
      <div className="topbar">
        <div className="brand">
          <div className="brand-mark">{brand.brandLetter}</div>
          <div className="brand-text"><h1>{brand.brandTitle}</h1><p>{brand.brandSubtitle}</p></div>
        </div>
